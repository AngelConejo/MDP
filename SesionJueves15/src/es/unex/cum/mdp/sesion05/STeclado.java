...
public class STeclado {
	...
	private static STeclado st = null;
	...
	public int Menu(String[] args, int min, int max) throws IOException {
		int opcion = 0;
		do {
			try { // Try de la excepción por si se introduce una letra en vez de un número en el
					// menú o no se introduce nada
				...
				opcion = Integer.parseInt(Input.readLine()); // Convertimos lo que leemos por teclado y lo convertimos a
																// entero
				System.out.print("\n");

			} catch (NumberFormatException e) { // Excepción por si se introduce una letra en vez de un número en el
												// menú o no se introduce nada
